...
- [10. Preprocessing Service 상세 설계](#10-preprocessing-service-상세-설계)
...
| 구현 기준 | 중앙 MediaMTX, 최대 4개 활성 Camera, RTSP/HLS/Recording, Preprocessing/Data/External 분리, Edge Pairing·상태·복구 |
| 문서 상태 | 현재 구현 정합화 설계안 |
...
- 중앙 Nginx, MediaMTX, Preprocessing, Data, External Service
...
- Web UI·iOS 제품화 (Android `mobile/` 통합은 포함)
- Cloud 기반 대규모 분산 처리
...
이전 프로토타입은 작업 폴더에서 삭제했고 현재 서비스의 실행·빌드에 포함하지 않는다.
...
| 공개 REST/HLS 계약 | [docs/external-app-integration.md](../external-app-integration.md), `/api/v1/openapi.json` |
| 공개 API Request/Response 검증 | `server/services/external/app/schemas.py` |
| 내부 Data API 검증 | `server/services/data/app/schemas.py` |
| 영속 DB Schema | `server/services/data/app/database/migrations/*.sql` |
...
- Preprocessing Service는 영상 소비, 탐지, 추적, 박스·크롭과 AI Event 생성을 담당한다.
- Preprocessing 내부 Identity는 전역 인물 연결 블랙박스, 별도 Analysis는 추가 분석·metadata 생성 블랙박스다. 두 모델의 구현은 별도 담당자가 맡는다. [상세 계약](../object-processing.md).
...
External, Preprocessing과 MediaMTX 제어·HLS 원본 Port는 Docker 내부 Network에서만 접근한다.
...
flowchart LR
    ...
    subgraph Central["Central Server"]
        ...
        Media -->|RTSP| Preprocessing
        Media -->|fMP4 recording| Recordings
        Media -->|recording hook| Nginx
        Preprocessing -->|Event / identity| Nginx
        Analysis -->|Analysis job / result| Nginx
...
| Preprocessing | `server/services/preprocessing/` | Camera Worker·추적·인물 연결 작업 | Event·관측·이미지·전역 ID |
| Analysis | `server/services/analysis/` | metadata 분석 작업 | 분석 결과·처리 상태 |
...
| 8000/TCP | Data/External/Preprocessing | Docker 내부 `expose` | 각 Python Service |
...
- `person_id`는 한 Camera Worker의 추적 Session 안에서만 유효한 임시 ID다.
...
Preprocessing Supervisor도 활성 목록의 처음 4개만 Worker로 구성한다. Camera 비활성화는
...
├── secrets/preprocessing.env
├── secrets/analysis.env
├── secrets/media.env
...
- RTSP Read는 Publish 자격증명과 분리된 Preprocessing Reader 자격증명을 사용한다.
...
## 10. Preprocessing Service 상세 설계

### 10.1 모듈

| 모듈 | 책임 |
|---|---|
| `app/settings.py`, `app/data_client.py` | 감지 설정·내부 Data 통신 |
| `app/supervisor.py`, `app/pipeline.py`, `app/event_state.py` | 카메라 작업기·영상 수신·등장/사라짐·관측 전송 |
| `processors/detection/contracts.py` | `DetectionFrame` → `DetectionResult` v1 계약 |
| `processors/detection/yolo.py` | 교체 가능한 기존 YOLO/ByteTrack 기본 구현 |
| `processors/identity/` | 전역 인물 연결 블랙박스 |
| `app/main.py` | 감지·인물 연결 작업기의 독립 시작·종료와 상태 확인 |
| `ai_cctv_core.processing.worker` | 임대 작업 수신·크롭 경로 검증·실행 제한·결과 반환 |

감지의 `DETECTION_PLUGIN`, 인물 연결의 `IDENTITY_PLUGIN`으로 담당 구현체를 선택한다. 세부 계약은 [Preprocessing 개발 안내](../../server/services/preprocessing/README.md)를 따른다.
...
5. Detection을 `person_id`, `confidence`, `bbox`로 정규화한다.
...
기본값은 Confidence 0.4, 분석 5fps, 사라짐 판정 3초다. `person_id`는 영구적인 사람
식별자가 아니며 Process 재시작, Stream 재연결과 장시간 가림 후 재사용을 보장하지 않는다.

### 10.4 Event Payload

Preprocessing가 `POST /internal/data/v1/events`로 보내는 기준 Payload는 다음과 같다.
...
Preprocessing Stream 장애 Event는 Edge의 중앙 연결 장애와 다르며 Recovery Job을 만들지 않는다.

### 10.6 AI Attribute 확장

metadata 분석은 `server/services/analysis/`의 독립 컨테이너가 수행한다. `ANALYSIS_PLUGIN`으로 선택한 `process(job, crop_path)` 결과를 Data에 반환하며, Data는 기존 객체·인물 연결 정보를 유지한 채 `metadata.analysis`에 병합한다.

기본 구현 `MetadataBlackBox`는 `unconfigured`를 반환한다. 의상·행동·기타 속성이나 점수는 생성하지 않는다. 담당 개발자는 결과 내부 스키마와 모델을 정하고 JSON 64 KiB 제한·유한 숫자·전역 ID 할당 금지 계약을 지킨다. [Analysis 개발 안내](../../server/services/analysis/README.md)와 [공통 객체 계약](../object-processing.md)을 따른다.
...
| 모듈 | 책임 |
|---|---|
| `main.py`, `dependencies.py`, `config.py` | 생명주기·DB 의존성·운영 설정 |
| `api/` | 사용자·세션·카메라·이벤트·녹화·객체·알림·유지보수 내부 API |
| `security.py`, `errors.py`, `schemas.py` | 호출자 Scope·오류 변환·요청 검증 |
| `database/connection.py`, `database/migrations/` | SQLite 연결·트랜잭션·버전 이관 |
| `database/repositories/` | 도메인별 SQL·관계·상태 전이 |
| `storage/` | 경로·녹화 파일·보관 기간 관리 |
| `workers/` | 복구·정합성 점검·주기 정리 |

이벤트·객체 관측·후속 작업 생성은 같은 DB 트랜잭션을 유지한다.
...
| `DATA_INFERENCE_TOKEN` | Preprocessing 감지 | 활성 Camera 조회, 상태·좌표 갱신, Event 생성 |
| `DATA_IDENTITY_TOKEN` | Preprocessing 인물 연결 | identity 작업 임대·완료·재처리 |
| `DATA_ANALYSIS_TOKEN` | Analysis | analysis 작업 임대·완료·재처리 |
...
External은 사용자 로그인·카메라 권한을 적용하는 공개 애플리케이션 계층이다.

| 모듈 | 책임 |
|---|---|
| `main.py` | 라우터 조합·상태 수집·FCM 작업기 생명주기 |
| `api/` | 로그인·사용자·카메라·이벤트·녹화·객체·알림·미디어 인증 |
| `security/` | Argon2·JWT·로그인 지연·Principal·ACL |
| `clients/` | Data·Edge·MediaMTX 통신 |
| `notifications/firebase.py` | FCM 발송 어댑터 |
| `workers/` | 상태 수집·푸시 발송 |
| `camera_lifecycle.py` | 카메라 제어와 게시 인증의 공유 잠금 |
| `dependencies.py`, `errors.py`, `schemas.py` | 설정·클라이언트 제공·오류 변환·공개 계약 |

구체적인 경계와 보안 규칙은 [External 개발 안내](../../server/services/external/README.md)를 따른다.
...
| MediaMTX | Preprocessing | RTSP/1.0 TCP, H.264 | Preprocessing Reader Credential | Video Stream |
...
정확한 Request/Response는 OpenAPI와 [외부 연동 계약](../external-app-integration.md)을
정본으로 사용한다.
...
| --- | --- | --- |
| Camera Discovery | `GET /cameras/enabled` | Preprocessing |
| Camera Status | `PATCH /cameras/{id}/status` | Preprocessing/External |
| Runtime/Profile | `GET/PUT /cameras/{id}/runtime-status`, `GET/PATCH .../video-profile` | External |
| Event | `POST /events`, `GET /events` | Preprocessing/External |
...
snapshots/<camera_id>/YYYY/MM/DD/<timestamp>_<person_id>.jpg
database/ai_cctv.db
...
```mermaid
sequenceDiagram
    participant I as Preprocessing Worker
    ...
    N->>D: Event JSON + Preprocessing Token
    D->>DB: 겹치는 Segment 조회
...
Camera 입력, 중앙 연결과 Preprocessing 소비 상태를 하나의 Boolean으로 합치지 않는다.

### 17.2 Preprocessing Track 상태

Worker는 `person_id → (last_seen_monotonic, confidence)` Map을 유지한다.
...
- Preprocessing Worker Dictionary는 Supervisor Thread만 조정한다.
...
| Preprocessing | `/health/live` | Process 생존 | Data 접근과 Supervisor 상태 |
...
| Preprocessing RTSP Read | 1초부터 최대 15초 지수 Backoff |
| Preprocessing Event POST | 경고 후 다음 Event 처리, Worker 유지 |
...
`server/compose.yml`은 `data`, `external`, `preprocessing`, `analysis`, `mediamtx`, `nginx` 여섯 Container를 정의한다. 모든 Service는 `restart: unless-stopped`와 Init Process를 사용한다.
Data, MediaMTX와 Preprocessing은 Host UID/GID를 사용하여 Bind Mount 권한 충돌을 줄인다.
...
| Snapshots | `/snapshots` | Preprocessing/Data Read/Write |
| Models | `/models` | Preprocessing Read-only |
...
| Preprocessing | 활성 Camera API, RTSP H.264, Model | Event JSON, Snapshot 상대 경로, 상태 Event | Worker 격리, 재접속, Health |
...
- `camera_id`, UTC `occurred_at`, `person_id`, `confidence`의 의미와 범위를 유지한다.
...
새 Model이 전역 Person Re-identification을 제공하더라도 `person_id`의 기존 의미를 바꾸지
않는다. 카메라 간 동일 인물 연결에는 별도 `global_person_id`를 사용한다. 현재 판정기는 블랙박스이며 미설정 상태에서는 이 값이 null이다.
...
| `server/services/data/tests/test_data_service.py` | Migration, CRUD, 검색, 오류, Retention/Reconciliation |
| `tests/test_recovery_coordinator.py` | 장애 병합, Manifest, SHA-256, Retry, 멱등성 |
| `server/services/external/tests/test_external_service.py` | JWT, RBAC, Camera/Media/Edge 조정, 공개 API |
| `server/services/preprocessing/tests/test_preprocessing.py` | Worker, Track Event, Snapshot, Stream 실패 |
...
- Preprocessing Event Request와 Data Validation
...
2. [MP4 Mock Edge](../../tools/mock_edge/README.md) 또는 실제 Edge를 Pairing하여 Camera를 등록한다.
3. Edge가 RTSP H.264를 게시하고 MediaMTX가 HLS와 Recording을 생성한다.
4. Preprocessing가 사람 출현/퇴장 Event와 Snapshot을 저장한다.
...
| AI Event | `FR-AI-001~013`, `FR-MODEL-001~008` | Preprocessing/Model Manager Test |
...
| 항목 | 현재 상태 | 후속 설계 조건 |
| --- | --- | --- |
| Camera 수 | 최대 4개 활성 | 부하 시험, Preprocessing Scheduling, DB 확장 검토 |
...
| Person ID | Session 범위 `person_id` | Re-identification은 별도 개인정보·정확도 설계 필요 |
