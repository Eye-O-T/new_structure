...
- [4. 이전 구조 정리](#4-이전-구조-정리)
- [5. 목표 시스템 컨텍스트](#5-목표-시스템-컨텍스트)
- [6. 논리 서비스와 컨테이너](#6-논리-서비스와-컨테이너)
- [7. Edge 아키텍처](#7-edge-아키텍처)
- [8. 주요 데이터 흐름](#8-주요-데이터-흐름)
- [9. 데이터 아키텍처](#9-데이터-아키텍처)
- [10. API 경계](#10-api-경계)
- [11. 인증과 보안 아키텍처](#11-인증과-보안-아키텍처)
- [12. Docker Compose 배포](#12-docker-compose-배포)
- [13. 설정 아키텍처](#13-설정-아키텍처)
- [14. 설치와 배포 아키텍처](#14-설치와-배포-아키텍처)
- [15. 장애 모델과 복구 전략](#15-장애-모델과-복구-전략)
- [16. 관측성과 진단](#16-관측성과-진단)
- [17. 성능과 확장 기준](#17-성능과-확장-기준)
- [18. 현재 코드에서 목표 구조로의 마이그레이션](#18-현재-코드에서-목표-구조로의-마이그레이션)
- [19. 테스트 아키텍처](#19-테스트-아키텍처)
- [20. 보류 및 미결정 사항](#20-보류-및-미결정-사항)
- [21. 최종 목표 구조 요약](#21-최종-목표-구조-요약)
...
애플리케이션 책임은 **영상, 감지·추적, 전역 인물 연결, 객체 추가 분석, 데이터, 외부**로 구분하고, Nginx는 공통 인프라로 둔다. 전역 인물 판정은 다른 개발자가 구현할 블랙박스로 분리한다.
...
| AD-013 | PyQt Configurator는 설치·운영 설정 도구이고 관제 화면은 Android 앱에서 제공한다. | 서버 운영과 일반 사용자 영상 관제를 분리하기 위함이다. |
...
이전 단일 카메라 프로토타입은 작업 폴더에서 삭제했다. 현재 운영은 Edge·중앙 6개 컨테이너·모바일·Configurator 구조를 기준으로 한다. 이전 구현은 Git 기록에서만 참고하며 실행·빌드·의존성에 포함하지 않는다.
...
flowchart LR
    ...
    N -->|public /hls| M
    N -->|public /playback| M
    I -->|internal event / identity HTTP| N
    A -->|internal analysis job / result| N
...
- Data Service, Preprocessing Service, MediaMTX HLS/Playback 내부 포트는 Host에 공개하지 않는다.
...
중앙 Compose는 **6개 컨테이너**로 구성한다.

| 컨테이너 | 책임 |
|---|---|
| `preprocessing` | 영상 감지·추적·박스·크롭, 별도 작업기로 전역 인물 연결 |
| `analysis` | 객체 입력을 받아 metadata 결과 반환 |
| `data` | SQLite 단독 접근, 이벤트·관측·작업·결과 저장 |
| `external` | 인증·권한·공개 API·Edge 상태 수집·FCM 발송 |
| `mediamtx` | RTSP·HLS·녹화·Playback |
| `nginx` | HTTPS·공개 요청·내부 서비스 요청 중계 |

Preprocessing과 Analysis는 독립 배포 단위다. 인물 연결은 Preprocessing 내부 작업기로 실행하며 metadata 분석은 별도 Analysis 컨테이너에서 실행한다. 두 알고리즘은 기본 `unconfigured` 블랙박스다. [객체 처리 계약](../object-processing.md)을 따른다.
...
영상 감지·추적·박스·크롭을 만드는 카메라 작업기와 전역 인물 연결 작업기를 한 컨테이너에서 실행한다. 감지는 카메라별 스레드이며 인물 연결은 별도 비동기 작업과 모델 스레드이므로 인물 연결 실패가 카메라 감지를 막지 않는다.

- `processors/detection/`: 교체 가능한 v1 입력·출력 계약과 기존 YOLO/ByteTrack 기본 어댑터
- `processors/identity/`: `global_person_id` 결과를 반환하는 블랙박스
- `app/`: 설정·감독·영상 수신·이벤트 전달·상태 확인

감지 모델이 없으면 가짜 결과를 만들지 않고 상태와 영상 연결을 점검한다. 인물 연결 기본 구현은 `unconfigured`다. SQLite는 직접 열지 않으며 Data에 HTTP로 관측과 결과를 전달한다.

**Analysis 서비스**는 객체 작업과 크롭을 받아 `metadata.analysis`에 저장할 결과를 반환하는 독립 컨테이너다. 기본 `MetadataBlackBox`는 `unconfigured`이며 모델 선택과 속성 스키마는 담당 개발자가 구현한다. `global_person_id`를 할당하지 않는다.

개발 지점과 검증 계약은 [Preprocessing](../../server/services/preprocessing/README.md), [Analysis](../../server/services/analysis/README.md)에 정의한다.
...
Nginx는 외부 listener와 Docker Network 전용 내부 listener를 별도 `server` block으로 구성한다. Preprocessing, External, Media hook의 제어·메타데이터 HTTP 요청은 내부 listener를 거쳐 Data Service로 중계한다. Data Service 내부 Recovery Worker는 Loopback API를 사용한다. 내부 listener는 Host에 publish하지 않는다. Nginx를 영상 프레임이나 대용량 파일 업로드의 서비스 버스로 사용하지 않는다.
...
| Android 사용자 애플리케이션 | `mobile/`, 중앙 REST와 보호된 HLS/Playback 사용 |
| Web UI·iOS 제품화 | 현재 검증 범위 제외 |

선택적 FCM은 [모바일 푸시 계약](../mobile-push.md)을 따른다. Data가 사용자·세션에 연결한
기기와 이벤트별 발송 대기열을 SQLite에 저장하고, External이 lease를 받아 권한이 유효한
수신자에게 발송한다. 별도 DB·메시지 브로커·공개 테스트 서버는 추가하지 않는다.
...
sequenceDiagram
    participant M as MediaMTX
    participant I as Preprocessing Service
...
Preprocessing Service 장애는 MediaMTX의 녹화와 HLS를 중단시키지 않는다.
...
Edge Publisher가 생성한 `central_connection_lost/restored`만 자동 Segment 복구의 권위 Event다. MediaMTX를 읽는 Preprocessing Worker의 장애는 `inference_stream_lost/restored`로 별도 기록하여 복구 구간을 만들지 않는다. 중복 수집 또는 순서가 뒤바뀐 동일 장애 보고는 상관 Window 안에서 시작 최솟값과 종료 최댓값으로 병합한다.
...
`PUBLIC_BASE_URL`이 설정된 운영 배포는 Live HLS와 Playback에 Absolute HTTPS URL을 반환한다. 개발용 미설정 배포만 같은 Origin 기준 상대 경로를 반환할 수 있다. 외부 앱 계약의 정본은 [외부 애플리케이션 REST/HLS 연동](../external-app-integration.md)이다.
...
| POST | `/events` | Preprocessing 또는 External (`DATA_INFERENCE_TOKEN`/`DATA_EXTERNAL_TOKEN`) |
| GET | `/cameras/enabled` | Preprocessing (`DATA_INFERENCE_TOKEN`) |
| PATCH | `/cameras/{id}/status` | Preprocessing (`DATA_INFERENCE_TOKEN`) |
...
Preprocessing, Media hook, Recovery Worker는 자기 Token 하나만 받으며 환경 HTTP Proxy와
...
Preprocessing에만 동일하게 주입한다. Preprocessing은 이 값을 percent-encode한 URI userinfo로
MediaMTX에 제시하며, Data와 Media hook에는 reader 쌍을 주입하지 않는다.

### 10.3 Media 제어 API

External Service의 좁은 Media Adapter만 MediaMTX Control API를 사용한다. 호출은 Nginx Docker 전용 `/internal/media/` route를 거치며, Data Service와 Preprocessing Service가 MediaMTX 설정 형식을 직접 알지 않도록 한다. 현재 Adapter 책임은 Camera disable/delete 때 `GET /v3/paths/get/{camera_id}`로 활성 RTSP source를 찾고 `POST /v3/rtspsessions/kick/{session_id}`로 해당 publisher만 종료하는 것이다.
...
현재 Edge Telemetry, Event, Availability와 Command/Result는 인증된 HTTP 상태·제어 API를 사용한다. MQTT는 현 릴리스의 구성 요소나 필수 의존성이 아니다. 후속 도입 시 Broker, 장치 인증, retained 상태, Last Will, QoS, Command ID 기반 중복 제거와 결과 Topic을 함께 설계하고 HTTP 계약에서 단계적으로 이전한다. 영상은 MQTT로 전달하지 않으며 Edge→중앙·Preprocessing은 계속 RTSP, 사용자 재생은 계속 HLS/Playback을 사용한다.
...
RTSP publish credential은 Edge 설정의 secret 영역에 저장하고 로그나 CLI 일반 출력에 노출하지 않는다. Preprocessing read credential은 게시 자격증명과 별도로 생성하며 32자 이상의 password를 External과 Preprocessing secret 파일에만 동일하게 저장한다. External 인증 callback은 read 요청에서 이 전용 쌍만 constant-time 비교하고 Camera publish 자격증명을 read 권한으로 재사용하지 않는다.
...
전체 저장소 구조는 [루트 README](../../README.md)를 따른다.

### 12.2 Compose 개념 구조

실행 정본은 [`server/compose.yml`](../../server/compose.yml)이다. 여섯 서비스 모두 독립 실행·상태 확인·재시작 정책을 가지며, 영속 데이터는 Host 디렉터리에 Bind Mount한다. 예제 YAML을 복사해 별도 배포 정의를 만들지 않고 정본과 Configurator가 생성한 경로 설정을 사용한다.

- Data: DB·녹화·복구·스냅샷 저장소 관리
- MediaMTX: 녹화 저장소 쓰기, 이벤트 완료 Hook
- Preprocessing: 모델 읽기, 스냅샷·크롭 쓰기
- Analysis: 모델·스냅샷 읽기 전용
- External: Data API와 FCM SDK 사용, DB 직접 접근 없음
- Nginx: 인증서 읽기, API·영상 요청 중계

선택적 `compose.push.yml`은 External에 FCM 설정을 추가하며 컨테이너 수를 늘리지 않는다.
...
- `media_net`: MediaMTX ↔ Preprocessing/Nginx
...
| 8554 | RTSP | 기본 loopback, 원격 Edge 필요 시 명시적 trusted-LAN IP | Edge publish, Preprocessing authenticated read |
...
일반 설정은 `config/config.yaml`, Compose 경로·포트·플러그인 선택은 `config/compose.env`에 둔다. 기존 설정 스키마의 `inference` 구역과 `INFERENCE_*` 환경변수명은 유지한다.
...
운영자가 모델 파일을 별도로 다운로드
  ...
  -> Preprocessing에 models 디렉터리를 읽기 전용 마운트
  -> Preprocessing restart
```
...
| Preprocessing 장애 | 이벤트 생성 중단 | health/worker status | 추론 재시작, 영상 녹화는 지속 |
...
| Preprocessing | event loop/thread | 모델 load + Media 연결 + Nginx 내부 Data route 가능 |
...
Container·Audio·파일시스템 overhead는 표에 포함하지 않으므로 실제 저장소에는 최소 10~20% 여유와 보관 정책을 반영한다. HLS egress는 동시 사용자 한 명당 선택 Profile Bitrate가 대략 추가된다. MediaMTX→Preprocessing의 RTSP는 Docker 내부 대역폭으로 별도 발생하며 추론 FPS를 낮춰도 RTSP 원본 전송 Bitrate 자체가 자동으로 줄지는 않는다. 실제 용량과 성능은 코덱, GOP, 장면 복잡도와 동시 재생 수에 따라 측정한다.
...
이번 구조 정리에서는 기존 프로토타입을 제거하고 서비스별 API·저장·보안·작업 실행 책임을 분리했다. 레거시 알고리즘을 자동으로 이전하지 않으며, 전역 인물 연결과 metadata 분석은 담당 개발자가 정해진 계약 뒤에서 구현한다.

기존 설치는 `COMPOSE_PROJECT_NAME`을 유지하고 `up --remove-orphans`로 이전 서비스 컨테이너를 정리한다. Bind Mount 데이터는 보존한다. 업데이트 순서는 DB 백업 → `enable_object_processing.py`로 기존 자격 증명 보존 이관 → Compose 재생성 → API·영상·저장소 인수 확인이다. 단순 `restart`는 변경된 서비스 정의와 환경파일을 반영하지 않는다. SQL 마이그레이션은 Data 시작 시 적용된다.

새 모듈 구조와 서비스별 테스트 위치는 [각 서비스 README](../../README.md#서비스와-컨테이너)와 [객체 처리 계약](../object-processing.md)을 따른다.
...
Raspberry Pi 1..4
  ...
  - recording / playback
       | RTSP          | files
       v               v
Preprocessing Service   Persistent Storage
       | event HTTP         |
       v                    | segment hook
