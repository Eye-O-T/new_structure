...
본 명세서는 중앙 MediaMTX·멀티카메라 구조의 기능·운영 요구사항을 정의한다. 이전 프로토타입은 작업 폴더에서 삭제했으며 실행·빌드 기준에 포함하지 않는다.
...
- iOS 모바일 앱 제품화 (Android 앱은 `mobile/`에 통합)
- Web UI
- MQTT Broker와 MQTT 기반 Telemetry·Event·Availability·Command

단, HTTPS, JWT, 비밀번호 해시, 비밀정보 보호는 저장 영상 암호화와 별개로 현재 범위에 포함한다. Android 앱은 `mobile/`에 통합하며 기존 HTTPS REST, 보호된 HLS/Playback과 OpenAPI 계약을 사용한다. 모든 이벤트를 기본 수신하는 선택적 FCM은 사용자·Camera ACL 및 로그인 세션에 연결한다. Firebase 설정과 실기기 검증은 별도 인수 조건이다. MQTT 도입 전 Edge 상태 조회·화질 제어·복구는 인증된 HTTP를 사용한다.
...
현재 기준 구현은 다음 특성을 가진다. 이전 단일 카메라 관제 코드는 작업 폴더에서 삭제했다.
...
| GUI/CLI | PyQt/CLI Configurator와 별도 Android 관제 앱 |
| 멀티카메라 | 최대 4개 활성 Camera ID/Stream Path; 이력 보존용 비활성 Camera는 별도 |
| Docker Compose | Data·External·Preprocessing·Analysis·MediaMTX·Nginx 6개 |
...
| 배포 컨테이너 | MediaMTX, Preprocessing, Data, External, Nginx |
...
| Preprocessing Service | RTSP 스트림을 읽고 탐지·추적·박스·크롭을 생성하는 서비스 |
...
| Android 앱 | `mobile/`, 중앙 HTTPS 계약과 선택적 FCM 사용 |
...
flowchart LR
    ...
    M -- RTSP --> I[Preprocessing Service]
...
| FR-RECOVERY-013 | Preprocessing의 MediaMTX 소비 장애는 `inference_stream_lost/restored`로 구분하며 Edge Segment 복구를 시작하지 않아야 한다. |
...
| FR-MEDIA-013 | MediaMTX RTSP read는 Camera 게시 자격증명과 다른 Preprocessing 전용 reader 자격증명을 요구하고 External 인증 callback과 Preprocessing만 그 자격증명을 공유하여야 한다. |
...
| ID | 요구사항 |
| --- | --- |
| FR-AI-001 | Preprocessing Service는 MediaMTX의 RTSP 스트림을 직접 읽어야 한다. |
...
| FR-AI-005 | 추론 이벤트는 Camera ID, Event Type, 발생 시각, person_id, 선택 global_person_id, Confidence를 포함하여야 한다. |
...
| FR-AI-014 | 전역 인물 연결과 객체 metadata 분석은 각각 Identity·Analysis 컨테이너의 블랙박스 인터페이스로 분리하고, 담당자가 구현한 결과를 Data에 반영하여야 한다. |
| FR-AI-015 | 모바일 실시간 화면에 객체 위치와 인물 ID를 표시하고 오래된 관측을 숨겨야 한다. HLS와 좌표의 시차를 사용자에게 안내한다. |
...
| FR-MODEL-005 | 모델은 Docker Volume을 통해 Preprocessing Service에 읽기 전용으로 마운트하는 것을 권장한다. |
...
| FR-NGINX-011 | Nginx는 `/internal/` 경로를 Docker 내부 네트워크에만 제공하고 Media, Preprocessing, External Service의 제어·메타데이터 요청을 Data Service로 중계하여야 한다. |
...
| Preprocessing read 인증 | 전용 `MEDIA_READ_USERNAME`/`MEDIA_READ_PASSWORD`; URI userinfo percent-encoding |
...
| --- | --- | --- |
| `person_appeared`, `person_disappeared` | 사람 Track 전이 | `person_id`, `confidence` |
...
| `inference_stream_lost`, `inference_stream_restored` | MediaMTX→Preprocessing 소비 전이; 복구 비권위 | `reason` |
...
- Preprocessing Service → Nginx 내부 경로 → Data Service: 이벤트·관측·좌표 전송, 인물 연결 작업 수신·결과 반환
- Analysis Service → Nginx 내부 경로 → Data Service: metadata 작업 수신·결과 반환
...
내부 API는 Docker Network에서만 접근 가능하여야 한다. External·감지·인물 연결·분석·Media Hook·Recovery에는 서로 다른 Token을 주입하여 Route allowlist를 강제한다. 감지와 인물 연결이 같은 Preprocessing 컨테이너에 있어도 토큰은 분리한다. 모든 내부 HTTP Client는 환경 프록시와 Redirect를 사용하지 않아야 한다.
...
| `person_id` | text | nullable, 카메라 내 추적 ID |
| `global_person_id` | text | nullable, 여러 카메라의 동일 인물 연결 ID; 현재 미할당 |
...
이 표는 Container·Audio·파일시스템 overhead를 제외한다. 실제 할당에는 최소 10~20% 여유와 보관 정책을 반영한다. HLS egress는 동시 시청자마다 해당 Profile Bitrate가 대략 추가되며 MediaMTX→Preprocessing 내부 RTSP도 별도 트래픽이다.
...
| NFR-SEC-011 | Preprocessing RTSP reader 비밀번호는 32자 이상이어야 하며 External/Preprocessing 파일 간 일치, 서비스별 allowlist와 URL userinfo percent-encoding을 검증하여야 한다. |
...
| 파일 | 허용 Secret | 주입 대상 |
| --- | --- | --- |
| `data.env` | 서로 다른 External/Detection/Identity/Analysis/Media/Recovery Data Token, 초기 관리자 ID/Argon2 Hash, 필요 시 Legacy Edge Token map | Data |
| `external.env` | `DATA_EXTERNAL_TOKEN`, JWT Signing Secret, Bootstrap RTSP 게시 자격증명 map, Preprocessing 전용 RTSP reader 쌍 | External |
| `preprocessing.env` | `DATA_INFERENCE_TOKEN`, `DATA_IDENTITY_TOKEN`, External과 동일한 RTSP reader 쌍 | Preprocessing |
| `analysis.env` | `DATA_ANALYSIS_TOKEN` | Analysis |
...
신규 설치는 단일 `secrets.env`를 모든 Container에 공유하지 않는다. 여섯 Data Token은 32자 이상이고 상호 달라야 하며 Data API는 Token별 허용 Route를 강제하여야 한다. RTSP reader username/password는 External과 Preprocessing에만 동일하게 저장하고 Data/Media에는 없어야 하며 password는 32자 이상이어야 한다. Compose와 `doctor`는 기존 결합 파일, 서비스 allowlist 위반, reader 쌍 누락·불일치를 운영 입력으로 허용하지 않고, `INTERNAL_SERVICE_TOKEN` 호환 처리는 Compose 밖의 직접 개발·테스트에만 유지한다. 실제 Secret 값은 Git에 Commit하지 않아야 하며 POSIX `0600` 또는 Windows에서 상속을 제거한 명시적 DACL로 보호하여야 한다.
...
기본 Compose는 다음 6개 컨테이너로 구성한다:

```text
mediamtx          # 영상 논리 서비스
preprocessing     # 감지·추적·박스·인물 연결
analysis          # metadata 분석 블랙박스
