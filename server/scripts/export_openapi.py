...
def build_document() -> dict[str, Any]:
    ...
    document["x-generation"] = {
        "command": "python server/scripts/export_openapi.py",
        "check": "python server/scripts/export_openapi.py --check",
...
def render_document() -> str:
    return (
        "# 자동 생성: python server/scripts/export_openapi.py (직접 수정하지 않음)\n"
        + yaml.dump(
            build_document(),
...
def main() -> int:
    ...
    if args.check:
        if not target.exists() or target.read_text(encoding="utf-8") != expected:
            print("OpenAPI differs; run python server/scripts/export_openapi.py")
            return 1
