...
from server.services.external.app.clients.data import (
    DataClient,
    DataConflict,
    DataForbidden,
    DataNotFound,
...
from server.services.external.app.clients.edge import EdgeControlError, EdgeHttpClient
from server.services.external.app.clients.mediamtx import (
    MediaControlError,
    MediaMtxClient,
)
...
from server.services.external.app.security.tokens import (
    TokenExpiredError,
    decode_token,
    issue_token,
)
...
def service(password_hash: str, settings: Settings, monkeypatch):
    ...
    monkeypatch.setattr(
        "server.services.external.app.api.cameras._disconnect_camera_publisher",
...
def test_video_profile_updates_current_only_after_edge_applied(
    ...
    monkeypatch.setattr(
        "server.services.external.app.api.cameras.EdgeHttpClient", FakeEdge
    )
...
def test_video_profile_maps_unavailable_capabilities_before_empty_profile_list(
    ...
    monkeypatch.setattr(
        "server.services.external.app.api.cameras.EdgeHttpClient", UnavailableEdge
    )
    response = client.patch(
        "/api/v1/cameras/cam-001/video-profile",
        headers={"Authorization": f"Bearer {token}"},
...
async def test_video_profile_lock_keeps_concurrent_edge_and_data_state_consistent(
    ...
    monkeypatch.setattr(
        "server.services.external.app.api.cameras.EdgeHttpClient", SerializedEdge
    )
    admin_token = issue_token(
        settings,
        user_id="1",
...
def test_video_profile_does_not_duplicate_edge_journaled_rejection(
    ...
    monkeypatch.setattr(
        "server.services.external.app.api.cameras.EdgeHttpClient", RejectedEdge
    )
    response = client.patch(
        "/api/v1/cameras/cam-001/video-profile",
        headers={"Authorization": f"Bearer {token}"},
...
def test_video_profile_rejects_cross_wired_edge_capability(
    ...
    monkeypatch.setattr(
        "server.services.external.app.api.cameras.EdgeHttpClient", WrongCameraEdge
    )
    response = client.patch(
        "/api/v1/cameras/cam-001/video-profile",
        headers={"Authorization": f"Bearer {token}"},
...
def test_live_object_api_requires_camera_acl(service):
    ...
    _login(client)
    allowed = client.get("/api/v1/cameras/cam-001/objects")
    assert allowed.status_code == 200
    assert allowed.headers["cache-control"] == "no-store"
    assert client.get("/api/v1/cameras/cam-002/objects").status_code == 403
