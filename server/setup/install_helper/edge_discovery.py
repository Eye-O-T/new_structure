...
# 검증을 통과한 광고 정보이며 주소는 광고 본문 대신 실제 UDP 발신자로 채운다.
...
# JSON의 true/false가 Python 정수로 취급되므로 불리언을 먼저 제외한다.
    ...
    *,
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
        ...
        while len(results) < max_results:
            ...
            try:
                # 상한보다 한 바이트 더 받아 잘린 초과 패킷을 정상 크기로 오인하지 않게 한다.
...
# 검증된 관리 주소에 초기 설정 완료 경로를 붙여 게시 계정을 전달할 대상을 만든다.
