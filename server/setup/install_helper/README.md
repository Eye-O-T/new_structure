...
인물 식별 모델은 별도 선택란 또는 CLI의 `--identity-model <파일.onnx>`으로 지정한다.
탐지 모델과 식별 모델은 같은 `models/` 폴더에 복사하므로 대소문자를 구분하지 않고
서로 다른 파일명을 사용해야 한다. 충돌하면 운영 폴더나 파일을 만들기 전에 중단한다.
미지정 시 저장소의 `models/osnet_x0_25_msmt17.onnx`, 서버 패키지의 `runtime/models/`,
`models/` 순으로 찾는다. 256 MiB 이하 ONNX가 없으면 설치를 시작하지 않는다.
`.onnx.json` 출처 기록과 `.LICENSE.txt` 파일이 있으면 모델과 함께 복사한다.
...
위 배포 대조·제한 시간·상태 요약은 GUI 서비스 작업에 적용한다. CLI `init`·`install`도 기존 설정·DB·녹화 또는 설치 중단 표식이 있으면 기본적으로 거부한다. 기존 설치의 시작·업데이트는 `start`, 장애 확인은 `doctor`를 사용한다. `init`은 Docker 없이 파일을 준비할 수 있으며 TLS 파일을 생략하면 서비스 시작 전에 별도로 준비해야 한다.

기존 설정을 의도적으로 다시 만들 때만 `init --reset-existing` 또는 `install --reset-existing`을 사용한다. 이 옵션은 기존 파일의 `.bak`을 남기고 **서비스 토큰·JWT 키·초기 카메라 게시 계정을 재발급**하므로 사전에 운영 데이터를 백업하고 서비스·Edge 자격 증명을 함께 정비해야 한다. 반복하면 직전 `.bak`이 교체된다. 기존 관리자 계정과 비밀번호는 DB에 남으며 이 옵션으로 비밀번호를 변경하지 않는다. 중단 표식이 남은 설치는 백업·진단으로 복구 상태를 확인하기 전까지 재설정하지 않는다.
...
전체 `doctor`는 컨테이너 상태와 별도로 Preprocessing 준비 상태, 카메라별 모델 준비·프레임 최신 여부, 인물 식별 작업자의 준비·정지 상태, 감지 이벤트 outbox의 대기·거부 건수를 검사한다. HTTP 200이어도 `degraded`이거나 이 항목에 문제가 있으면 경고한다. 잠시 쌓인 대기 건수는 다음 진단에서 해소되는지 확인한다. 상세 오류 원문이나 인증값은 이 진단 출력에 포함하지 않는다.
...
설치본에는 [운영·복원 절차](../../../docs/operations.md)와 릴리스 기록 도구도 같은 상대
경로로 포함한다. `server/secrets/` 전체와 `.bak` 백업은 배포에서 제외하고, 서비스별
지정된 다섯 `.env.example`만 포함한다. `.bak`은 Git에서도 제외한다. 빌드 후 설치 파일을
추출해 실제 비밀값이나 운영 백업이 포함되지 않았는지 확인한다. 소스의 포함·제외 규칙
검사는 실제 EXE의 설치·업데이트·제거 검증을 대신하지 않는다.
