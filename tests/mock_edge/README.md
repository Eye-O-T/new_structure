...
테스트 전용 Edge입니다. `FFmpeg -stream_loop -1 -re`로 MP4를 실시간 속도로
반복 재생하여 중앙
...
MediaMTX에 게시한 영상을 중앙 Preprocessing Service가 읽어 추론합니다.
...
테스트 영상은 직접 준비한 MP4를 지정합니다. 예시 경로는 실제 파일 위치로 바꿉니다.

```text
C:\test-videos\sample.mp4
```
...
PC에 Python·FFmpeg를 설치하지 않고 모의 Edge를 실행할 수도 있다. 저장소 루트에서 이미지를 만든다.

```powershell
docker build -f tests/mock_edge/Dockerfile -t ai-cctv-mock-edge .
docker run --rm ai-cctv-mock-edge --help
```

Windows Docker Desktop에서 직접 등록 자격 증명을 사용하는 예시다. 아래 폴더에 MP4·Pairing Key·중앙에서 발급한 게시 자격 증명을 준비한다. 두 HTTP 포트는 신뢰 LAN 방화벽 범위를 설정한다.
...
`host.docker.internal`은 Docker Desktop 호스트를 가리킨다. 원격 중앙 서버라면 실제 LAN 주소로 바꾼다. 중앙에서 접근할 관리·복구 주소도 Docker 호스트의 도달 가능한 주소로 등록한다. 브리지 네트워크의 UDP 자동 검색은 호스트 직접 실행과 다르므로 이 예시는 직접 등록을 사용한다. 상태·녹화는 별도 Docker 볼륨에 보존된다.
